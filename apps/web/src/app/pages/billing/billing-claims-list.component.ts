...
@Component({
          ...
          @for (c of claims(); track c.id) {
            <div class="claim-card">
              ...
              <div class="claim-body">
                ...
                @if (c.amountApproved !== undefined) { <p><strong>Approved:</strong> \${{ c.amountApproved | number }}</p> }
                @if (c.amountPaid !== undefined) { <p><strong>Paid:</strong> \${{ c.amountPaid | number }}</p> }
              </div>
              <div class="claim-actions">
                @if (c.status === 'draft' && canSubmitPay()) {
                  <button (click)="submit.emit(c.id)" class="action-btn">Submit</button>
                }
  ...
  styles: [`
    ...
    .claim-list { display: flex; flex-direction: column; gap: 0.75rem; margin-top: 1rem; }
    .claim-card { padding: 1rem; background: #f5f5f5; border-radius: 6px; }
    .claim-header { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem; flex-wrap: wrap; }
    .claim-type { font-weight: 600; color: #1a237e; text-transform: uppercase; font-size: 0.75rem; }
    .claim-amount { margin-left: auto; font-weight: 600; }
    .claim-status {
      padding: 0.15rem 0.4rem; border-radius: 4px; font-size: 0.7rem;
      text-transform: uppercase; font-weight: 600;
    }
    .claim-status.draft { background: #f5f5f5; color: #666; }
    .claim-status.submitted { background: #fff3e0; color: #e65100; }
    .claim-status.pending { background: #e3f2fd; color: #1565c0; }
    .claim-status.adjudicated { background: #e8f5e9; color: #2e7d32; }
    .claim-status.partial { background: #fff8e1; color: #f57f17; }
    .claim-status.denied { background: #ffebee; color: #c62828; }
    .claim-status.paid { background: #e8f5e9; color: #1b5e20; }
    .claim-body p { margin: 0.25rem 0; font-size: 0.875rem; }
    .claim-actions { margin-top: 0.75rem; display: flex; gap: 0.5rem; }
    .action-btn {
